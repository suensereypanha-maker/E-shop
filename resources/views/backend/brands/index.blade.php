@extends('backend.layouts.app', ['title' => 'Brands Management'])
...
@section('content')
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-200">
            <div>
                <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-slate-700 font-semibold">Brands</span>
                </div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Brands Management</h1>
            </div>
    ...
    <div id="addModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 w-full max-w-lg p-6 relative max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                <h3 class="text-base font-bold text-slate-900">Add New Brand</h3>
                <button type="button" onclick="closeAddModal()" class="text-slate-400 hover:text-slate-600">
                    <i class="bi bi-x-lg text-sm"></i>
                </button>
            </div>
            <form action="{{ route('backend.brands.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Brand Name <span class="text-rose-500">*</span></label>
                    <input type="text" name="name" required placeholder="e.g. Nike, Apple, Adidas"
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Logo Image</label>
                    <div class="flex items-center gap-3">
                        <div id="add_logo_preview_wrapper" class="w-12 h-12 rounded border border-slate-200 bg-slate-50 flex items-center justify-center overflow-hidden hidden">
                            <img id="add_logo_preview" src="" alt="Preview" class="w-full h-full object-contain">
                        </div>
                        <input type="file" name="logo" accept="image/*" onchange="previewImage(this, 'add_logo_preview', 'add_logo_preview_wrapper')"
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Brief description of the brand..."
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs">Save Brand</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 w-full max-w-lg p-6 relative max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                <h3 class="text-base font-bold text-slate-900">Edit Brand</h3>
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Logo Image</label>
                    <div class="flex items-center gap-3">
                        <div id="edit_logo_preview_wrapper" class="w-12 h-12 rounded border border-slate-200 bg-slate-50 flex items-center justify-center overflow-hidden hidden">
                            <img id="edit_logo_preview" src="" alt="Brand Logo" class="w-full h-full object-contain">
                        </div>
                        <input type="file" name="logo" accept="image/*" onchange="previewImage(this, 'edit_logo_preview', 'edit_logo_preview_wrapper')"
                            class="flex-1 text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer">
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    <button type="button" onclick="closeEditModal()"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-xs font-medium hover:bg-slate-50">Cancel</button>
                    <button type="submit" class="px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs">Update Brand</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function openEditModal(brand) {
            ...
            document.getElementById('edit_status').checked = (brand.status == 1 || brand.status === true);

            var previewWrapper = document.getElementById('edit_logo_preview_wrapper');
            var previewImg = document.getElementById('edit_logo_preview');
            if (brand.logo) {
            ...
            var baseUrl = "{{ url('backend/brands') }}";
            document.getElementById('editForm').action = baseUrl + '/' + brand.id;
