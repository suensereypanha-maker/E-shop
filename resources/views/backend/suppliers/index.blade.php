@extends('backend.layouts.app', ['title' => 'Suppliers Management'])
...
@section('content')
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-200">
            <div>
                <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-slate-700 font-semibold">Suppliers</span>
                </div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Suppliers Management</h1>
            </div>
    ...
    <div id="addModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 w-full max-w-lg p-6 relative max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                <h3 class="text-base font-bold text-slate-900">Add New Supplier</h3>
                <button type="button" onclick="closeAddModal()" class="text-slate-400 hover:text-slate-600">
                    <i class="bi bi-x-lg text-sm"></i>
                </button>
            </div>
            <form action="{{ route('backend.suppliers.store') }}" method="POST" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Supplier Name <span class="text-rose-500">*</span></label>
                        <input type="text" name="name" required placeholder="e.g. John Doe, Alpha Trading"
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Supplier Code</label>
                        <input type="text" name="code" placeholder="Auto-generated if empty (e.g. SUP-0001)"
                            class="w-full text-xs px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="company_name" placeholder="e.g. Acme Corporation"
                            class="w-full text-xs px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Contact Person</label>
                        <input type="text" name="contact_person" placeholder="e.g. Sarah Connor"
                            class="w-full text-xs px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="address" rows="3" placeholder="Office / Warehouse address..."
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs">Save Supplier</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 w-full max-w-lg p-6 relative max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                <h3 class="text-base font-bold text-slate-900">Edit Supplier</h3>
            ...
            <form id="editForm" method="POST" class="space-y-4">
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    <button type="button" onclick="closeEditModal()"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-xs font-medium hover:bg-slate-50">Cancel</button>
                    <button type="submit" class="px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs">Update Supplier</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function openEditModal(supplier) {
            ...
            document.getElementById('edit_status').checked = (supplier.status == 1 || supplier.status === true);

            var baseUrl = "{{ url('backend/suppliers') }}";
            document.getElementById('editForm').action = baseUrl + '/' + supplier.id;
