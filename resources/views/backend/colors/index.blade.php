@extends('backend.layouts.app', ['title' => 'Colors Management'])
@push('styles')
...
@section('content')
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-200">
            <div>
                <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-slate-700 font-semibold">Colors</span>
                </div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Colors Management</h1>
            </div>
    ...
    <div id="addModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 w-full max-w-md p-6 relative">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                <h3 class="text-base font-bold text-slate-900">Add New Color</h3>
            ...
            <form action="{{ route('backend.colors.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Color Name <span class="text-rose-500">*</span></label>
                    <input type="text" name="name" required placeholder="e.g. Navy Blue, Crimson Red"
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Color Code (HEX)</label>
                    <div class="flex items-center gap-2">
                        <input type="color" id="add_picker" value="#3b82f6" oninput="document.getElementById('add_code').value = this.value"
                            class="w-9 h-9 p-0.5 rounded border border-slate-300 cursor-pointer">
                        <input type="text" name="code" id="add_code" value="#3b82f6" placeholder="#000000" oninput="document.getElementById('add_picker').value = this.value"
                            class="flex-1 text-xs px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none font-mono">
                    </div>
                </div>
                <div class="flex items-center gap-2">
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs">Save Color</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs hidden">
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 w-full max-w-md p-6 relative">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
                <h3 class="text-base font-bold text-slate-900">Edit Color</h3>
            ...
            <form id="editForm" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Color Code (HEX)</label>
                    <div class="flex items-center gap-2">
                        <input type="color" id="edit_picker" oninput="document.getElementById('edit_code').value = this.value"
                            class="w-9 h-9 p-0.5 rounded border border-slate-300 cursor-pointer">
                        <input type="text" name="code" id="edit_code" placeholder="#000000" oninput="document.getElementById('edit_picker').value = this.value"
                            class="flex-1 text-xs px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none font-mono">
                    </div>
                </div>
                <div class="flex items-center gap-2">
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    <button type="button" onclick="document.getElementById('editModal').classList.add('hidden')"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-xs font-medium hover:bg-slate-50">Cancel</button>
                    <button type="submit" class="px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs">Update Color</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        function openEditModal(color) {
            ...
            document.getElementById('edit_code').value = color.code || '';
            if (color.code && color.code.startsWith('#')) {
                document.getElementById('edit_picker').value = color.code;
            }
            document.getElementById('edit_status').checked = (color.status == 1 || color.status === true);
            var baseUrl = "{{ url('backend/colors') }}";
            document.getElementById('editForm').action = baseUrl + '/' + color.id;
